...
describe("useBalance", () => {
  ...
  describe("initial loading state", () => {
    it("should start in loading state when address is provided", async () => {
      const { result } = renderHook(
        () => useBalance({ address: TEST_ADDRESS }),
        { wrapper }
      );
  ...
  describe("successful balance retrieval", () => {
    it("should fetch XLM balance successfully", async () => {
      const { result } = renderHook(
        () => useBalance({ address: TEST_ADDRESS, asset: "XLM" }),
        { wrapper }
      );
    ...
    it("should fetch issued asset balance successfully", async () => {
      ...
      const { result } = renderHook(
        () => useBalance({ address: TEST_ADDRESS, asset }),
        { wrapper }
      );
    ...
    it("should fetch liquidity pool share balance successfully", async () => {
      ...
      const lpBalance = result.current.balances.find(b => 
        typeof b.asset === "string" && b.asset === "liquidity_pool_shares"
      );
  ...
  describe("error handling", () => {
    it("should handle account not found error", async () => {
      mockHorizonServer.loadAccount.mockRejectedValue(new Error("Request failed with status code 404"));
    ...
    it("should handle unexpected SDK errors", async () => {
      mockHorizonServer.loadAccount.mockRejectedValue(new Error("Network Error"));
